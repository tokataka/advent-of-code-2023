...
pub fn solution(lines: Vec<&str>) -> String {
    ...
    let (root_size, size_list) = unsafe { (*file_system).size_list() };

    let mut size_list: Vec<_> = size_list.into_iter().filter(|x| (root_size - *x) <= 40000000).collect();
    size_list.sort();

    size_list[0].to_string()
}
